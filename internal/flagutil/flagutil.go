// Package flagutil provides shared helpers for Go's stdlib flag package. It
// is used by both the CLI (cmd/sluice) and the Telegram command handler
// (internal/telegram) so the two surfaces accept the same flag shapes and
// behave identically on edge cases like the "--" terminator and single-dash
// short flags.
package flagutil
