...
- **One credential = at most one pool** (post-merge hardening, Copilot review): `PoolResolver.PoolForMember` maps a member back to a single pool, so a credential shared across pools would persist/audit a token response against the wrong pool's phantom (agent left with an unreplaceable phantom). `CreatePoolWithMembers` rejects a member already in another pool, enforced in the same transaction as the member insert. API-host failover attribution is also pinned to the member injected for that request (recovered by flow ID) rather than the response-time active member, so concurrent in-flight 429s cannot cool an innocent member.
